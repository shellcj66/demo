...
jQuery(function($) {
	...
	function enableAjax() {
		//防止直接根目录访问出错
		if(document.location.pathname.indexOf("index.html")<0) window.location.href = "index.html#page/main";
		   ...
		   $('[data-ajax-content=true]').ace_ajax({
			 ...
			 'content_url': function(hash) {
				...
				if(path.match(/(index\.html)?/))
					return path.replace(/index\.html?/, 'page/'+hash.replace(/^page\//, '')+'.html');
			  },
...
function switchFilter(e){
	$(e).find("span").toggleClass('rotate180');
	$(e).parent().siblings(".adv-filter").toggleClass('hide');
}
